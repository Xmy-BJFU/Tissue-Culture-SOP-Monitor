...
class TQDM:
    ...
    def _display(self, final: bool = False) -> None:
        ...
        # Clip to terminal width. If the line wraps, \r only returns to the last
        # wrapped row, so Cursor/PowerShell looks like it prints a new line every step.
        try:
            cols = max(40, shutil.get_terminal_size(fallback=(120, 24)).columns)
        except Exception:
            cols = 120
        max_len = cols - 1
        if len(progress_str) > max_len:
            # Keep the right side (percent + bar + it/s), that is the "loading" part
            progress_str = progress_str[-max_len:]

        try:
            if self.noninteractive:
                self.file.write(progress_str)
            else:
                self.file.write(f"\r\033[K{progress_str:<{max_len}}")
            self.file.flush()
        except Exception:
            pass
